...
namespace Silkke
{
    ...
    public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        void Update()
        {
            // Return if ScrollSnapRect not initialized
            if (!initialized)
                return;

            // Prev AND Next buttons handler
            prevButton.SetActive(GetNearestPage() - 1 >= 0 ? true : false);
            nextButton.SetActive(GetNearestPage() + 1 <= _pageCount - 1 ? true : false);

            if (Input.GetKeyDown(KeyCode.RightArrow))
            {
                if (GetNearestPage() + 1 <= _pageCount - 1)
                    NextScreen();
            }

            if (Input.GetKeyDown(KeyCode.LeftArrow))
            {
                if (GetNearestPage() - 1 >= 0)
                    PreviousScreen();
            }
        ...
        private void SetPagePositions()
        {
            ...
            for (int i = 0; i < _pageCount; i++)
            {
                ...
                if (_horizontal)
                {
                    ...
                    childPosition = new Vector2(i * (childWidth + (_scrollRectRect.rect.width / 6f)), 0f);
                }
                else
                    childPosition = new Vector2(0f, -(i * height - containerHeight / 2 + offsetY));
                child.anchoredPosition = childPosition;
                _pagePositions.Add(-childPosition);
                _buttons.Add(i, child.GetComponent<Button>());
            }

            // Bind btn onClick with their index
            foreach (KeyValuePair<int, Button> btn in _buttons)
            {
                btn.Value.onClick.AddListener(() => ClickOnBtn(btn.Key));
                if (btn.Key == startingPage)
                    btn.Value.GetComponent<avatarBtn>().updateBtnState(true);
                else
                    btn.Value.GetComponent<avatarBtn>().updateBtnState(false);
            }
        }

        private void updatePagePositions()
        {
            Debug.Log("update page positions");
            ...
            for (int i = 0; i < _pageCount; i++)
            {
                ...
                {
                    float childWidth = child.sizeDelta.x;
                    childPosition = new Vector2(i * (childWidth + (_scrollRectRect.rect.width / 5f)), 0f);
                }
                else
                    childPosition = new Vector2(0f, -(i * height - containerHeight / 2 + offsetY));
                child.anchoredPosition = childPosition;
        ...
        //------------------------------------------------------------------------
        private void ScaleNearestPage()
        {
            // Left button (if there is one)
            if (GetNearestPage() - 1 >= 0)
                _buttons[GetNearestPage() - 1].GetComponent<avatarBtn>().updateBtnState(false);

            // Middle button
            _buttons[GetNearestPage()].GetComponent<avatarBtn>().updateBtnState(true);

            // Right button (if there is one)
            if (GetNearestPage() + 1 < _pageCount)
                _buttons[GetNearestPage() + 1].GetComponent<avatarBtn>().updateBtnState(false);
        }

        //------------------------------------------------------------------------
        private void ScaleNearestPage(int btn_index)
        {
            _audioSource.Play();

            // Left button (if there is one)
            if (btn_index - 1 >= 0)
                _buttons[btn_index - 1].GetComponent<avatarBtn>().updateBtnState(false);

            // Middle button
            _buttons[btn_index].GetComponent<avatarBtn>().updateBtnState(true);

            // Right button (if there is one)
            if (btn_index + 1 < _pageCount)
                _buttons[btn_index + 1].GetComponent<avatarBtn>().updateBtnState(false);
        }

        //------------------------------------------------------------------------
        private void ClickOnBtn(int btn_index)
        {
            if (btn_index > _currentPage)
                NextScreen();
            else if (btn_index < _currentPage)
                PreviousScreen();
        }
